  ...
  useEffect(() => {
    if (grupoSeleccionado) {
      fetch(
        ...
        .then((res) => res.json())
        .then((data) => {
          // Filtramos alumnos ya con préstamos
          const filtrados = data.filter(
            (a) => !uidsConPrestamo.includes(a.uid)
          );
...
export function DialogoPrestarLibros({
  ...
  const handlePrestar = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/db/prestamos/prestarUsuario`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
