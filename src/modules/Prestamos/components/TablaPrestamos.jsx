...
export function TablaPrestamos({
  ...
  // Cursos únicos para el filtro select
  const cursosUnicos = Array.from(new Set(data.map((p) => p.curso).filter(Boolean))).sort();
    ...
    <div>
      <div className="flex flex-wrap gap-4 py-2 text-sm text-muted-foreground items-end">
        <div className="space-y-1">
          ...
          <select
            ...
            onChange={(e) =>
              table.getColumn("curso")?.setFilterValue(e.target.value || undefined)
            }
          >
                    ...
                    >
                      <TableCell className="w-10 text-center">
                        <Button
                          variant="ghost"
                          size="icon"
                          onClick={(e) => toggleExpandClick(usuario.id_prestamo, e)}
                        >
                    ...
                    {isExpanded &&
                      usuario.prestamos.map((item) => (
                        <TableRow key={item.id_item} className="bg-gray-50">
                          ...
                          <TableCell>
                            {item.fechaentrega || "-"} / {item.fechadevolucion || "-"}
                          </TableCell>
                        </TableRow>
                      ))}
                  </Fragment>
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between items-center py-6 space-y-4 sm:space-y-0">
        <div className="flex gap-2">
          {acciones && acciones(selectedId ? data.find(u => u.id_prestamo === selectedId) : null)}
        </div>
