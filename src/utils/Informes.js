...
/*export function generatePermisosPdf({ empleado, fecha }) {
  ...
  // Mostrar la fecha pasada como parámetro en formato dd/mm/yyyy
  ...
  doc.text(fechaStr, tableX + 30, y + row6Height - 3); // ajusta 30 según espacio
...
export function generatePermisosPdf({ empleado, fecha }) {
  ...
  opciones.forEach((opcion, index) => {
    ...
    if (opcion === tipoEmpleadoMap[empleado.tipo_empleado]) {
      doc.text("X", tableX + xPos + 0.3, checkY + 2.8); // marca con equis en el recuadro
    }
